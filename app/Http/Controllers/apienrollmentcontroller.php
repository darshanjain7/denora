  ...
  public function enrollmentuserpasschange(Request $request)
     {
            ...
            $pass_hint=$request->new_pass;
            $conf_new_pass=$request->conf_new_pass;
            $userid=$request->userid;
            $hashed = Hash::make($new_pass);
            
        $data = DB::update("UPDATE `users` SET  `password`='$hashed', `pass_hint`= '$pass_hint' WHERE `id`='$userid'");
           if($data>0)
           {
              $response = array('Response' => true);
       	    return json_encode($response);
...
public function enrollmentuserforgotpass(Request $request)
    {
        ...
        {
            $message->from("noreply.gaschlorination@denora.com");
		    $message->to('rajat.arokiait@gmail.com');
		  //  $message->to($data111);
